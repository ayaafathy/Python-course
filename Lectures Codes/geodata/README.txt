...
In this project, we are using the Google geocoding APIto clean up some user-entered geographic locations of
...
You should install the SQLite browser to view and modifythe databases from:
...
This program will read the input lines in where.data and for each line check to see if it is already
...
into the Places API.  Also all the Google Geo-related APIs require an API key.
To complete this assignment without a Google account,
without an API key, or from a country that blocks access to Google, you can use a subset of that data which is
available at:
